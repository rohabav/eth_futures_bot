...
from config import BINANCE_FAPI_BASE, BINANCE_API_KEY, BINANCE_API_SECRET, SYMBOL, TARGET_LEVERAGE, MARGIN_TYPE
...
class BinanceFuturesClient:
    ...
    def get_klines(self, symbol: str, interval: str, limit: int = 500) -> List[List[Any]]:
        return self._request("GET", "/fapi/v1/klines", signed=False,
                             params={"symbol": symbol, "interval": interval, "limit": limit})

    def get_order_book(self, symbol: str, limit: int = 20) -> Dict[str, Any]:
        return self._request("GET", "/fapi/v1/depth", signed=False,
                             params={"symbol": symbol, "limit": limit})

    def get_recent_trades(self, symbol: str, limit: int = 50) -> Any:
        return self._request("GET", "/fapi/v1/trades", signed=False,
                             params={"symbol": symbol, "limit": limit})

    def get_funding_rate(self, symbol: str, limit: int = 1) -> Any:
        return self._request("GET", "/fapi/v1/fundingRate", signed=False,
                             params={"symbol": symbol, "limit": limit})
...
def init_client() -> BinanceFuturesClient:
    client = BinanceFuturesClient(BINANCE_FAPI_BASE, BINANCE_API_KEY, BINANCE_API_SECRET)
    # Ensure margin type & leverage
